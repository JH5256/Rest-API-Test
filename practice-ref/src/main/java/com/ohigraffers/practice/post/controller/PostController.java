...
public class PostController {
    ...
    @Operation(summary = "전체 포스트 목록 조회", description = "전체 포스트 목록을 조회한다.")
        ...
        List<EntityModel<PostResponse>> postWithRel = posts.stream().map(
                post ->
                        EntityModel.of(
                                ...
                                linkTo(methodOn(PostController.class).findPostByCode(Math.toIntExact(post.getCode()))).withSelfRel()
                        )
        ).toList();
    ...
    /* Swagger 문서화 시 설명 어노테이션 작성 */
    /* RequestMapping 어노테이션 작성 */
    @Operation(summary = "특정 코드로 포스트 조회", description = "특정 코드를 통해 해당하는 포스트 정보를 조회")
    @RequestMapping("/findPosts/{code}")
    public ResponseEntity<ResponseMessage> findPostByCode(/* 필요 매개변수 선언 */ @PathVariable int code) {
   ...
   public ResponseEntity<Void> registPost(@Valid @RequestBody PostCreateRequest newPost) {
       ...
       return ResponseEntity
               .created(URI.create("/posts/registPost" + posts.get(posts.size() - 1).getCode()))
               .build();
   }
   ...
   /* RequestMapping 어노테이션 작성 */
    ...
    @RequestMapping("/modifyPost/{code}")
    public ResponseEntity<Void> modifyPost(@PathVariable int code, @Valid @RequestBody PostUpdateRequest updatePost) {
    ...
    @RequestMapping("/deletePost/{code}")
    public ResponseEntity<Void> removeUser(@PathVariable int code) {
